...
__myencfs_file_open(
	...
	if (strchr(mode, 'w') != NULL) {
		if (!_myencfs_util_createdir(system, base, file)) {
			goto cleanup;
		}
	}
...
myencfs_construct(
	...
	if ((myencfs->internal->crypto = _myencfs_crypto_new(myencfs->context)) == NULL) {
		_myencfs_error_entry_dispatch(__myencfs_error_entry_base(
			...
			_myencfs_error_entry_base(
				_myencfs_error_capture(myencfs_context_get_error(myencfs->context)),
				"myencfs_new.crypto",
	...
	if ((myencfs->internal->bio_random = _myencfs_bio_random(
		myencfs->context,
		myencfs->internal->crypto
	)) == NULL) {
		_myencfs_error_entry_dispatch(__myencfs_error_entry_base(
			myencfs,
			_myencfs_error_entry_base(
				_myencfs_error_capture(myencfs_context_get_error(myencfs->context)),
				"myencfs_new.bio_random",
				MYENCFS_ERROR_CODE_MEMORY,
				true,
				NULL
			)
...
myencfs_destruct(
	...
	if (myencfs != NULL) {
		...
		ret = myencfs_bio_destruct(myencfs->internal->bio_random) && ret;
		ret = _myencfs_crypto_destruct(myencfs->internal->crypto) && ret;

		ret = myencfs_system_free(system, "myencfs::base_ct", myencfs->internal->base_ct) && ret;
		ret = myencfs_system_free(system, "myencfs::base_pt", myencfs->internal->base_pt) && ret;
		ret = myencfs_system_free(system, "myencfs::md_suffix", myencfs->internal->md_suffix) && ret;
		ret = myencfs_system_free(system, "myencfs::key_id", myencfs->internal->encryption_key_id) && ret;
		ret = myencfs_system_free(system, "myencfs", myencfs) && ret;
	}
...
myencfs_get_bio_random(
	...
	return myencfs->internal->bio_random;
}
