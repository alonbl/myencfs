...
#include <myencfs/myencfs-system-driver-core.h>
...
static const struct myencfs_system_driver_entry_s __DRIVER_ENTRIES[] = {
	...
	{ MYENCFS_SYSTEM_DRIVER_ID_myencfs_access, (void (*)()) __driver_default_access},
	{ MYENCFS_SYSTEM_DRIVER_ID_myencfs_mkdir, (void (*)()) __driver_default_mkdir},
	{ 0, NULL}
};
...
myencfs_system_clean(
	...
	if (system != NULL) {
		myencfs_error error = system->error;
		system->error = NULL;
		_myencfs_error_destruct(error);
		memset(system, 0, sizeof(*system));
	}
...
myencfs_system_free(
	...
	myencfs_system_driver_core_free(system)(system, hint, p);
	return true;
}
