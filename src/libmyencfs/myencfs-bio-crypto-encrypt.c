...
__bio_encrypt_close(
	...
	if ((len = _myencfs_crypto_operation_encrypt_final(
		private->init->op,
		private->op->tail,
...
__bio_encrypt_destruct(
	...
	if (private != NULL) {
		...
		if (private->op->own_bios) {
			if (
				!myencfs_bio_destruct(private->op->bio_ct) ||
				!myencfs_bio_destruct(private->op->bio_md)
			) {
				ret = false;
			}
		}
		...
		if (!myencfs_system_free(system, "myencfs_bio_encrypt", private)) {
			ret = false;
		}
	}
...
__bio_encrypt_write(
	...
	if (private->op->tail_size != 0) {
		...
		if (private->op->tail_size == private->init->block_size) {
			if ((len = _myencfs_crypto_operation_encrypt_update(
				private->init->op,
				private->op->tail,
				private->op->tail_size,
				buffer_ct,
...
myencfs_bio_crypto_encrypt_init(
	...
	if (!_myencfs_crypto_rand_bytes(myencfs_internal->crypto, private->op->md->iv, iv_size)) {
		goto cleanup;
	}
	...
	if (!_myencfs_crypto_operation_encrypt_init(
		private->init->op,
		key, key_size,
