...
myencfs_bio_destruct(
	...
	if (bio != NULL) {
		...
		ret = myencfs_system_free(system, "myencfs_bio", bio) && ret;
	}
...
__mem_destruct(
	...
	if (private != NULL) {
		...
		if (!private->fixed) {
			ret = myencfs_system_free(system, "myencfs_bio_mem::buf", private->buf) && ret;
		}

		ret = myencfs_system_free(system, "myencfs_bio_mem", private) && ret;
	}
