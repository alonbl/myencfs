...
#define KEY_ID "keyid1"
	...
	if ((myencfs = myencfs_new(context)) == NULL) {
		fprintf(stderr, "myencfs new failed\n");
		goto cleanup;
	}

	if (!myencfs_construct(myencfs)) {
		fprintf(stderr, "myencfs construct failed\n");
		goto cleanup;
	}

	if (!myencfs_set_encryption_key_id(myencfs, KEY_ID)) {
		fprintf(stderr, "set key id failed\n");
		goto cleanup;
	}

	if (!myencfs_set_key_callback(myencfs, __key_callback)) {
		fprintf(stderr, "set key callback failed\n");
		goto cleanup;
	}

	if ((bio_random = myencfs_get_bio_random(myencfs)) == NULL) {
		goto cleanup;
	}
	...
	if (myencfs_bio_read(
		...
		fprintf(stderr, "key read failed\n");
		goto cleanup;
	}
	...
	if (myencfs_bio_read(
		...
		fprintf(stderr, "plaintext1 read failed\n");
		goto cleanup;
	}
	...
	if ((bio_pt1 = myencfs_bio_mem_buf(
		...
		fprintf(stderr, "bio_pt1 create failed\n");
		goto cleanup;
	}
	if ((bio_ct = myencfs_bio_mem(context, "bio_ct")) == NULL) {
		fprintf(stderr, "bio_ct create failed\n");
		goto cleanup;
	}
	if ((bio_md = myencfs_bio_mem(context, "bio_md")) == NULL) {
		fprintf(stderr, "bio_md create failed\n");
		goto cleanup;
	}
	...
	{
		unsigned char *p;
		size_t s = myencfs_bio_mem_get_data(bio_ct, (void**)&p);
		if (s != sizeof(plaintext1)) {
			fprintf(stderr, "ciphertext size is incorrect\n");
			goto cleanup;
		}
	...
	if ((bio_pt2 = myencfs_bio_mem(context, "bio_pt2")) == NULL) {
		fprintf(stderr, "bio_pt2 create failed\n");
		goto cleanup;
	}
	if (myencfs_bio_seek(bio_ct, 0, SEEK_SET) == -1) {
		fprintf(stderr, "bio_ct reset failed\n");
		goto cleanup;
	}
	if (myencfs_bio_seek(bio_md, 0, SEEK_SET) == -1) {
		fprintf(stderr, "bio_md reset failed\n");
		goto cleanup;
	}
