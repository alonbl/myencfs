...
test_sanity() {
	...
	for f in "${KEY_ID1}" "${KEY_ID2}"; do
		dd status=none if=/dev/urandom of="${KEY_STORE}/${f}" bs=1 count=$((256/8)) || die "Cannot generate '${f}'"
	done

	for f in ${FILES}; do
		dd status=none if=/dev/urandom of="${BASE_PT}/${f}" bs=1 count=$((1024*50)) || die "Cannot generate '${f}'"
	...
	dd status=none if=/dev/urandom of="${BASE_PT}/file4.bad" bs=1 count=$((1024*50)) || die "dd status=none failed"
	"${MYENCFS_TOOL}" encrypt \
		--key-store="${KEY_STORE}" \
		--key-id="${KEY_ID2}" \
		--base-pt="${BASE_PT}" \
	...
	dd status=none if=/dev/urandom of="${BASE_PT}/file5.missing" bs=1 count=$((1024*50)) || die "dd status=none failed"
