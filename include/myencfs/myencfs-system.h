...
#define MYENCFS_SYSTEM_DRIVER_FUNC(group, ret, name, ...) \
	typedef ret (*myencfs_system_driver_p_##name)(const myencfs_system system __VA_OPT__(,) __VA_ARGS__); \
	static inline myencfs_system_driver_p_##name myencfs_system_driver_##group##_##name (const myencfs_system system) { \
		return (myencfs_system_driver_p_##name)myencfs_system_driver_find(system, MYENCFS_SYSTEM_DRIVER_ID_##group##_##name); \
	}
#elif defined(HAVE_GCC_VARARGS_MACROS)
#define MYENCFS_SYSTEM_DRIVER_FUNC(group, ret, name, ...) \
	typedef ret (*myencfs_system_driver_p_##name)(const myencfs_system system, ##__VA_ARGS__); \
	static inline myencfs_system_driver_p_##name myencfs_system_driver_##group##_##name (const myencfs_system system) { \
		return (myencfs_system_driver_p_##name)myencfs_system_driver_find(system, MYENCFS_SYSTEM_DRIVER_ID_##group##_##name); \
	}
#else
